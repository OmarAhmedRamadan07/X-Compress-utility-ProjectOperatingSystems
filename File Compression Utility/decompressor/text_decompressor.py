...
def decompress_text_file(bin_path: str, code_path: str, output_path: str):
    try:
        logger.info("Starting text decompression")
        ...
        decoded_text = decode_data(encoded_data, root)

        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(decoded_text)

        logger.info(f"Text file decompressed → {output_path}")

    except FileNotFoundError as e:
        logger.error(str(e))
        raise CompressionError(str(e))
    except Exception as e:
        logger.exception("error during text decompression")
        raise CompressionError(str(e))
